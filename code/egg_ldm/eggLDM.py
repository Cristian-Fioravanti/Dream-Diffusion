...
class eLDM:
    ...
    def __init__(self, metafile, num_voxels, device=torch.device('cpu'),
                 ...
                 logger=None, ddim_steps=250, use_time_cond=False, clip_tune=True, cls_tune=False):
        ...
        config.model.params.unet_config.params.global_pool = False
        ...
        model.cond_stage_model = cond_stage_model(
            metafile, num_voxels, self.cond_dim, clip_tune=clip_tune, cls_tune=cls_tune)
...
class cond_stage_model(nn.Module):
    def __init__(self, metafile, num_voxels=440, cond_dim=1280, clip_tune=True, cls_tune=False):
        super().__init__()

        if metafile is not None:  # Must Be not None Cri
            model = create_model_from_config(
                metafile['config'], num_voxels)  # num_voxels=512
        ...
        self.egg_latent_dim = model.embed_dim  # Dani 256

        self.channel_mapper = nn.Sequential(
            nn.Conv1d(self.egg_seq_len,  # self.egg_seq_len = 128
                      self.egg_seq_len // 2, 1, bias=True),  # self.egg_seq_len // 2 =64
