...
def main(config):
    ...
    model_name = "checkpoint_epoch_19.pth"
    ...
    for step, batch in enumerate(eeg_latents_dataset_train):
        ...
        image_real = Image.fromarray(
            ((image / 2 + 0.5).clamp(0, 1) * 255).numpy().astype('uint8'))
        ...
        for t in tqdm(scheduler.timesteps):
            with torch.no_grad():
                noisy_residual = unet(
                    input, t, hidden_states.unsqueeze(0), return_dict=False)[0]
                prev_noisy_sample = scheduler.step(
                    noisy_residual, t, input).prev_sample
