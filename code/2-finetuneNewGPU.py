...
def main(config):
    ...
    unet, encoder, vae, scheduler, optimizer, projector1, clip_model, crop_transform = accelerator.prepare(
        unet, encoder, vae, scheduler, optimizer, projector1, clip_model, crop_transform)
    # For Epoch
    train_loss = 0.0
    try:
            ...
            for step, batch in enumerate(eeg_latents_dataset_train):
                ...
                with accelerator.accumulate(unet, encoder, projector1):
                    ...
                    timesteps = torch.randint(
                        0, 1000, (1,), device=latents.device)
        ...
        save_model(unet, encoder, vae, clip_model, projector1,
                   config, config.output_path, epoch=None)
        exit(0)
